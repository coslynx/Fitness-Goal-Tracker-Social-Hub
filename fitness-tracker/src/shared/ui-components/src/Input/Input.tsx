...
const Input: React.FC<InputProps> = ({
  ...
  const inputClasses = tw(
    `${inputStyles.base} ${error ? inputStyles.error : ''} ${disabled ? inputStyles.disabled : ''} ${
  ...
  return (
    <div className={tw`mb-4`}>
      {label && <label htmlFor="input" className={tw`block font-medium text-gray-700`}>{label}</label>}
      <input
        ref={inputRef}
        ...
        readOnly={readOnly}
      />
      {error && <span className={tw`text-red-500`}>Error: Please check your input.</span>}
    </div>
  );
};
